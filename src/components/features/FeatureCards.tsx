 ...
 *
 * Remodelled from the 21st.dev Features 1 and Features 2 (Tailark). An icon tile sits centred
 * over a faint ink-200 grid that fades out in a circle, then a title and a sentence, all
 * centred. Two styles:
 *
 ...
 *
 * The tiles are charcoal; the muted style takes white tiles, which sit calmer on its recessed
 * ground. One column on phones, three from lg (or two from md with `columns={2}`). Not links, so
...
export const FeatureCards = ({ items, variant = 'outline', columns = 3, ground, eyebrow, title, description, align, headingLevel = 'h2', className }: FeatureCardsProps) => {
          ...
          <StaggerItem as="li" key={itemTitle} className="min-w-0">
            <div className={cn('flex h-full flex-col items-center rounded-lg p-6 text-center md:p-8', CARD[variant])}>
              <div className="relative h-36 w-36 shrink-0">
                ...
                <IconTile id={icon} size="md" ground={ground ?? (variant === 'muted' ? 'white' : 'carbon')} className="absolute inset-0 m-auto" />
              </div>
