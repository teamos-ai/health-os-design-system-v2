 ...
 *
 * Each question is a row with a soft apricot plus on the right. Opening it turns the plus a
 * quarter turn into a cross and reveals the answer beside a small picture: image on the left,
      ...
      {items.map((item, i) => {
        ...
        return (
          <div key={item.question} className="border-b border-line">
            <Heading className="font-display text-subheading text-ink-900">
              <button
                type="button"
                id={triggerId}
                ...
                onClick={() => toggle(i)}
                className="group flex w-full items-center justify-between gap-6 rounded-md py-6 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink-900 focus-visible:ring-offset-2 focus-visible:ring-offset-paper"
              >
                <span>{item.question}</span>
                <span
                  aria-hidden
                  className={cn(
                    'flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-apricot-200 text-ink-900 transition-[transform,background-color] duration-md ease-out group-hover:bg-apricot-200/80',
                    isOpen && 'rotate-45'
                  )}
                >
            ...
            <AnimatePresence initial={false}>
              {isOpen && (
                ...
                >
                  <div className={cn('grid gap-4 pb-6 sm:gap-6 sm:pr-14', item.image && 'grid-cols-[5rem_minmax(0,1fr)] sm:grid-cols-[8rem_minmax(0,1fr)]')}>
                    {item.image && (
