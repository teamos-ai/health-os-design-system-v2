 ...
 * paper title strip the photo dissolves into (text never sits on the raw photo). Below: format and length, the
    ...
    <div className="flex aspect-[4/3] items-center justify-center rounded-md bg-brand-gradient-soft">
      <div className="relative flex h-5/6 aspect-[3/4] flex-col overflow-hidden rounded-md bg-surface shadow-md transition-[transform,box-shadow] duration-md ease-out group-hover:-translate-y-2 group-hover:shadow-lg">
        <div className="image-fade-b relative min-h-0 flex-1">
          <img src={cover.src} alt={cover.alt} loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover" />
        </div>
        <div className="bg-surface px-3 pb-2">
          <p className="font-sans text-label uppercase text-ink-500">{type}</p>
