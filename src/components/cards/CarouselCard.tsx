 ...
 * arrows (shown on hover, keyboard focus and always on touch screens) and dots; the lower
 * third of the photo dissolves into the card with the shared image fade, so the dots and the
 * text never sit on a raw photo. Below: a category, the title, a meta line, one sentence,
 * then a single figure and an open action named after the card.
 *
 * Inside a moving row (a Marquee), pass `photoControls="pointer"`: the arrows and dots still
 * work with a pointer or touch but leave the tab order, so each card is one keyboard stop.
 */
...
export interface CarouselCardProps {
  ...
  /** keyboard: photo arrows and dots in the tab order (default), or pointer and touch only */
  photoControls?: 'keyboard' | 'pointer';
  className?: string;
}
...
const arrow =
  ...
  'opacity-0 group-hover/photos:opacity-100 focus-visible:opacity-100 [@media(hover:none)]:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink-900 active:scale-[0.98]';

export const CarouselCard = ({ images, category, title, meta, description, figure, action, photoControls = 'keyboard', className }: CarouselCardProps) => {
  ...
  const tabIndex = photoControls === 'pointer' ? -1 : undefined;

  return (
    <article
      className={cn(
        'flex w-80 shrink-0 flex-col overflow-hidden rounded-lg border border-line bg-surface',
        className
      )}
    >
      <div className="group/photos relative h-48" role="group" aria-roledescription="carousel" aria-label={`${title} photos`}>
        ...
        {count > 1 && (
          <>
            <div className="absolute inset-x-3 top-1/2 z-10 flex -translate-y-1/2 justify-between">
              <button type="button" aria-label="Previous photo" tabIndex={tabIndex} onClick={() => go(-1)} className={arrow}>
                <ChevronLeft className="h-4 w-4" strokeWidth={1.75} aria-hidden />
              </button>
              <button type="button" aria-label="Next photo" tabIndex={tabIndex} onClick={() => go(1)} className={arrow}>
            ...
            <div className="absolute bottom-1 left-1/2 z-10 flex -translate-x-1/2">
              {images.map((img, i) => (
                <button
                  ...
                  tabIndex={tabIndex}
                  onClick={() => goTo(i)}
                  className="flex h-6 min-w-6 items-center justify-center rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink-900"
                >
