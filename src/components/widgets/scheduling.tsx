        ...
        {days.map((d, i) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                'relative flex aspect-square items-center justify-center rounded-md font-sans text-body transition-colors duration-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700/40',
                isToday ? 'bg-brand-gradient text-white' : isSel ? 'bg-lavender-50 text-lavender-700' : 'text-ink-900 hover:bg-ink-100'
              )}
            >
              {d}
        ...
        {slots.map((s, i) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                'rounded-md border px-3 py-2 font-sans text-body tabular-nums transition-colors duration-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700/40',
                s.taken
      ...
      <motion.li key={s.title} className="relative flex gap-3 pb-4 last:pb-0" {...pop(i, 0.12)}>
        ...
        <span
          className={cn(
            ...
            s.status === 'done' && 'bg-brand-gradient text-white',
            s.status === 'current' && 'border-2 border-lavender-400 bg-surface text-lavender-700',
    ...
    <ul className="flex flex-col gap-3 p-5">
      {features.map((f, i) => (
        <motion.li key={f} className="flex items-center gap-3 font-sans text-body text-ink-900" {...pop(i, 0.1)}>
          <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-rose-700 text-white">
      ...
      <li className="mt-1">
        <button
          type="button"
          className="h-11 w-full rounded-md bg-carbon font-display text-body text-white transition-colors hover:bg-carbon/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700/40 focus-visible:ring-offset-2"
        >
          {action}
        </button>
      </li>
        ...
        return (
          <motion.li key={it.id} className="flex items-center justify-between gap-4" {...pop(i, 0.1)}>
            ...
            <button
              ...
              className={cn(
                'relative h-6 w-11 shrink-0 rounded-full transition-colors duration-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700/40 focus-visible:ring-offset-2',
