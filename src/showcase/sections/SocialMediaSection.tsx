      ...
      <div className="grid gap-8 md:grid-cols-[1fr_0.6fr]">
        ...
        <Frame label="Story" size="1080 × 1920">
          <div className="relative flex aspect-[9/16] flex-col overflow-hidden rounded-lg border border-line bg-surface">
            <img src={thumb(STORY_BG)} alt="A lavender field at dusk with a wooden post" className="min-h-0 flex-1 object-cover" loading="lazy" decoding="async" />
            <div className="flex flex-col gap-3 bg-surface p-5">
              ...
              <span className="self-start rounded-md bg-carbon px-3 py-2 font-display text-body text-white">Start the check</span>
            </div>
          </div>
        </Frame>

        <Frame label="Link preview" size="1200 × 630" className="md:col-span-2">
          <div className="grid aspect-[1200/630] grid-cols-2 overflow-hidden rounded-lg border border-line bg-surface">
            <div className="flex flex-col justify-between p-8" style={{ background: PAPER_IVORY }}>
              <LogoMark size={32} />
              <div>
                <p className="font-sans text-label uppercase text-rose-700">Guide</p>
            ...
            <img src={thumb(LINK_IMG)} alt="Three women working together on a sofa with laptops and coffee" className="h-full w-full object-cover" loading="lazy" decoding="async" />
          </div>
        </Frame>
      </div>
    </Example>
