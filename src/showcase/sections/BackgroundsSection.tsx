...
const Chip = ({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) => (
  <button
    ...
    onClick={onClick}
    className={cn(
      'rounded-md border px-3 py-1 font-sans text-label capitalize transition-colors duration-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700/40',
...
export const ImageTile = ({
  ...
  <figure className="group flex flex-col gap-3">
    <a href={href} download title={`Download ${name}`} className="block overflow-hidden rounded-lg border border-line bg-surface-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700/40 focus-visible:ring-offset-2">
    ...
    <figcaption className="flex flex-col gap-2">
      ...
      <span className="font-sans text-label text-ink-600">
        <span className="uppercase text-rose-700">Suggested use</span> {suggestedUse}
      </span>
      <span className="font-sans text-label text-ink-500">{meta}</span>
      {note && <span className="font-sans text-label text-warning-700">{note}</span>}
    </figcaption>
  </figure>
);
        ...
        <div className="flex flex-col gap-3 border-b border-line bg-surface-2 p-4 md:p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="flex items-center gap-3 font-sans text-label text-ink-600" aria-live="polite">
              ...
              {(ratio || tone || subject) && (
                <button type="button" onClick={reset} className="rounded-md text-ink-900 underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700/40">
                  Clear
                </button>
              )}
            </span>
