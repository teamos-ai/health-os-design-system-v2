...
Every H1 uses one to three icons, and cards, bentos and posts use them on their own (`IconTile`). Search the icon library (`HEADLINE_TILE_LIST`, or the Icon library section) for the word or the object: 276 icons in 17 groups, and all 107 headline words point at an icon. An icon marked `planned` needs making before it goes live.

- **Model:** Tumai's tiles (Health OS Branding Guidelines, pages 11 and 12): one real object in warm studio light, cut out, sitting on the squircle. The ground comes from the `icon` tokens, never from the image: warm charcoal by default, or paper and white for light contexts, all three from the same cut-out.
- **The object is itself, in its own colour.** A love heart is red. A flame is a flame. Wood is wood, brass is brass, a stamped card is stamped in ink. **Never recolour an object into the brand palette**: apricot, rose and lavender belong to the interface, not to the things in the pictures, and an object repainted to match a page stops looking like the thing it is. Tumai's rule, 21 September 2026, after a simplified cream-and-apricot style was tried across forty-seven icons and rejected: the reference collection he supplied was about the 3D craft, not its orange palette.
...
- **Check:** sharp at 3× the tile size, no text, numbers or logos, reads on the charcoal, paper and white grounds at 40px (pale objects lean on the light grounds' contact shadow). Redo anything that doesn't. **Measure it rather than judge it:** the object's median tone against the charcoal ground, WCAG style. Under 2.0 and the object is a silhouette of itself there; add the id to `READS_BETTER_ON_PAPER` in `src/data/headline-tiles.ts`, which records the winning ground for every icon in both directions: a near-black object is a silhouette on charcoal, and a very pale one vanishes on paper.
- **Say the word, do not decorate it.** The test is whether someone who sees the object with the word covered arrives at the word. Fire says hot, a trophy says won, a boomerang says it comes back. A pomegranate does not say whole. A metaphor is fine beside a headline where the word is right there, and fails in a pill, a card header or a nav where the picture is doing the talking.
