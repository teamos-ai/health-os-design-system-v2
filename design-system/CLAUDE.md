...
10. **Nine card types.** Content, feature, service, pricing, resource, action, session, profile and steps. Each is styled for its job; do not force one into another's role. The carousel card drives photo-led rows such as the overview. For a website feature section, use `FeatureBento` in its photo, tint or quiet style.
...
14. **Motion is on by default and calm.** Use the motion library and the duration and easing tokens. Nothing people need can wait on an animation.
